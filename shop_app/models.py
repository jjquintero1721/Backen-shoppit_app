...
class ProductSubmission(models.Model):
    ...
    seller = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='product_submissions')
    name = models.CharField(max_length=100)
    slug = models.SlugField(blank=True, null=True)
    image = models.ImageField(upload_to="img")
    description = models.TextField(blank=True, null=True)
    ...
    admin_commission = models.DecimalField(max_digits=5, decimal_places=2, default=10.0, 
                                           help_text="Commission percentage for admin")
    product = models.OneToOneField(Product, on_delete=models.SET_NULL, null=True, blank=True, 
                                  related_name='submission')
    
    def __str__(self):
        return f"{self.name} by {self.seller.username} - {self.status}"
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            unique_slug = self.slug
            counter = 1
            while ProductSubmission.objects.filter(slug=unique_slug).exists():
                unique_slug = f"{self.slug}-{counter}"
                counter += 1
            self.slug = unique_slug
        super().save(*args, **kwargs)
